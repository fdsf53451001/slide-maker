...
if (process.env.SLIDE_MAKER_ENABLE_CODEX_SOFT_SANDBOX !== "1") {
  throw new Error(
    "Set SLIDE_MAKER_ENABLE_CODEX_SOFT_SANDBOX=1 to run this quota-consuming smoke test",
  );
}
...
async function api(path, options = {}) {
  ...
  if (!response.ok)
    throw new Error(`HTTP ${response.status} ${path}: ${(await response.text()).slice(0, 500)}`);
  return response;
}
...
try {
  ...
  project = await (
    await api(`/api/projects/${project.id}/slides/${slideId}`, {
      method: "PATCH",
      body: JSON.stringify({
        purpose: "確認真實圖片生成",
        content: "圖片生成，端到端成功",
        ...
        dataBasis: [],
        imagePrompt:
          "深海軍藍背景、中央柔和橘色光暈、現代簡潔簡報封面。畫面唯一文字必須逐字為：圖片生成，端到端成功。不要標誌、不要浮水印。",
      }),
    })
  ).json();

  const job = await (
    await api(`/api/projects/${project.id}/slides/${slideId}/generate`, {
      method: "POST",
  ...
  if (!completedJob || completedJob.status !== "completed") {
    throw new Error(
      `Image job ${completedJob?.status ?? "timed_out"}: ${completedJob?.errorCode ?? "NO_ERROR_CODE"}`,
    );
  }
  ...
  if (!version?.imagePath)
    throw new Error("Successful image job did not create a current slide asset");
