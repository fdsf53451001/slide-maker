...
try {
  ...
  if (codex?.maxConcurrency !== 2)
    throw new Error(`Expected tested concurrency 2, got ${codex?.maxConcurrency}`);
  let project = await (
    await api("/api/projects", {
      method: "POST",
      body: JSON.stringify({
        topic: "Codex 並行圖片生成驗證",
  ...
  const specs = [
    {
      purpose: "並行驗證 A",
      content: "Parallel lane A",
      narrative: "獨立的藍色資料管線",
      layoutHint: "左至右單一路徑",
      imagePrompt:
        "Create a polished 16:9 slide titled Parallel A with a blue data pipeline, no logos.",
      sourceIds: [],
    },
    {
      purpose: "並行驗證 B",
      content: "Parallel lane B",
      narrative: "獨立的紫色驗證管線",
      layoutHint: "中央驗證節點",
      imagePrompt:
        "Create a polished 16:9 slide titled Parallel B with a purple verification pipeline, no logos.",
      sourceIds: [],
    },
  ];
  for (let index = 0; index < 2; index += 1)
    project = await (
      await api(`/api/projects/${project.id}/slides/${project.slides[index].id}`, {
        method: "PATCH",
        body: JSON.stringify(specs[index]),
      })
    ).json();
  ...
  while (Date.now() < deadline) {
    ...
    if (jobs.some((job) => ["failed", "cancelled"].includes(job.status)))
      throw new Error(`Parallel job failed: ${jobs.map((job) => job.errorCode).join(",")}`);
    if (jobs.length === 2 && jobs.every((job) => job.status === "completed")) break;
    await new Promise((resolvePromise) => setTimeout(resolvePromise, 1_000));
  }
  const jobs = project.jobs.filter((job) => ids.has(job.id));
  if (!observedOverlap || !jobs.every((job) => job.status === "completed"))
    throw new Error("Codex jobs did not complete with observed overlap");
  for (const slide of project.slides) {
    const version = slide.versions.find((item) => item.id === slide.currentVersionId);
    if (!version) throw new Error("Missing parallel output");
  ...
  const overlapMs =
    Math.min(...intervals.map((item) => Date.parse(item.finishedAt))) -
    Math.max(...intervals.map((item) => Date.parse(item.startedAt)));
