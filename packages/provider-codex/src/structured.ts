...
export async function runCodexStructured(options: CodexStructuredOptions): Promise<unknown> {
  ...
  if (rootMetadata.isSymbolicLink() || !rootMetadata.isDirectory())
    throw new Error("CODEX_STRUCTURED_WORKSPACE_INVALID");
  const canonicalRoot = await realpath(root);
  const workspace = await mkdtemp(join(root, "job-"));
  if (!inside(canonicalRoot, await realpath(workspace)))
    throw new Error("CODEX_STRUCTURED_WORKSPACE_INVALID");
  ...
  for (const [index, sourcePath] of (options.imagePaths ?? []).entries()) {
    ...
    if (
      ...
      throw new Error("CODEX_STRUCTURED_IMAGE_INVALID");
  ...
  const result = await spawnWithArgv(
    ...
    [
      ...
      outputPath,
      "-c",
      `web_search=${JSON.stringify(options.webSearchMode ?? "disabled")}`,
