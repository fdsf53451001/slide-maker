...
describe("project contract", () => {
  ...
  it("migrates legacy jobs to lifecycle version 1", () => {
    ...
    (project.jobs as unknown[]).push({
      id: "legacy-job", projectId: project.id, slideId: project.slides[0]!.id, providerId: "mock-image",
      status: "running", attempt: 1, createdAt: now, updatedAt: now,
    });
    const migrated = parseProject(project);
    expect(migrated.jobs[0]).toMatchObject({ lifecycleVersion: 1, phase: "waiting_for_codex", progress: { step: 4, total: 6 } });
  });

  it("derives the workflow stage for projects saved before the two-step flow", () => {
    const untouched = createProject({ topic: "Legacy requirements" }) as unknown as Record<string, unknown>;
    delete untouched.workflowStage;
    expect(parseProject(untouched).workflowStage).toBe("requirements");

    const generated = createProject({ topic: "Legacy generated" }) as unknown as Record<string, unknown>;
    delete generated.workflowStage;
    (generated.jobs as unknown[]).push({
      id: "old-job", projectId: generated.id, slideId: (generated.slides as Array<{ id: string }>)[0]!.id,
      providerId: "mock-image", status: "completed", attempt: 1,
      createdAt: generated.createdAt, updatedAt: generated.updatedAt,
    });
