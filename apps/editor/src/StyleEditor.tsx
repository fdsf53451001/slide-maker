...
export function StyleEditor({
  ...
  useEffect(() => {
    ...
    void load().catch((reason: unknown) =>
      setError(reason instanceof Error ? reason.message : "載入風格失敗"),
    );
    return () => {
      current = false;
    };
  ...
  return (
    <main className="style-editor-page">
      <header className="style-header">
        ...
        <div>
          <strong>{styleId ? (style?.name ?? "載入中…") : "建立風格"}</strong>
          <small>
            進階風格設定 ·{" "}
            {historicalVersion
                ...
                onClick={() => {
                  ...
                  void api
                    ...
                    .catch((reason: unknown) =>
                      setError(reason instanceof Error ? reason.message : "複製失敗"),
                    )
              ...
              <button
                onClick={() => {
                  ...
                  void api
                    ...
                    .then(onSaved)
                    .catch((reason: unknown) =>
                      setError(reason instanceof Error ? reason.message : "還原失敗"),
                    )
                    .finally(() => setBusy(false));
                }}
              >
        ...
        <aside className="reference-panel">
          ...
          {!readOnly && (
            ...
            >
              ＋ 加入 PNG / JPG 參考圖
              <input
                disabled={draft.referenceImages.length >= 4}
                type="file"
