...
describe("Editor MVP navigation", () => {
  it("keeps a pristine extracted text layer pixel-identical until the first edit", () => {
    const box = {
      id: "text-1", text: "原始文字", x: 100, y: 80, width: 300, height: 60,
    ...
    const props = {
      background: "/clean.png", original: "/original.png", boxes: [box], canvasWidth: 1920, canvasHeight: 1080,
      selectedId: undefined, onSelect: vi.fn(), onChange: vi.fn(),
    };
    const view = render(<TextLayerCanvas {...props} pristine />);
    expect(screen.getByRole("img", { name: "文字抽離原始預覽" }).getAttribute("src")).toBe("/original.png");
    expect((screen.getByLabelText("可編輯簡報文字") as HTMLTextAreaElement).style.color).toBe("transparent");

    view.rerender(<TextLayerCanvas {...props} pristine={false} />);
    expect(screen.getByRole("img", { name: "文字抽離乾淨背景" }).getAttribute("src")).toBe("/clean.png");
    expect((screen.getByLabelText("可編輯簡報文字") as HTMLTextAreaElement).style.color).not.toBe("transparent");
  });
  ...
  it("shows the first reference image on a style-library card when legacy data has no cover id", async () => {
    ...
    const style = {
      ...
      name: "既有風格",
      system: false,
      referenceImages: [{ id: "legacy-cover", name: "cover.png", mediaType: "image/png" as const, assetPath: "assets/legacy-cover.png", createdAt: now }],
    ...
    expect(screen.getByRole("img", { name: "既有風格 封面" }).getAttribute("src")).toBe("/api/style-assets/legacy-cover");
  ...
  it("asks for a page purpose and calls the AI single-slide outline flow", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (path === "/api/providers") return Response.json([{ id: "mock-image", name: "Mock", availability: { status: "available" }, capabilities: { fullSlideGeneration: true } }]);
      if (path === "/api/styles") return Response.json([createDefaultStyle(now)]);
      if (path.includes("/readiness")) return Response.json({ providerId: "mock-image", status: "ready", blocking: false, requiresAcknowledgement: false, message: "Ready", checkedAt: now, expiresAt: now });
      if (path.endsWith("/slides/ai") && init?.method === "POST") {
        const generated = { ...structuredClone(project.slides[0]!), id: "ai-generated-slide", order: 1, purpose: "比較導入前後成效", versions: [] };
        project = { ...project, slides: [project.slides[0]!, generated, { ...project.slides[1]!, order: 2 }] };
    ...
    fireEvent.change(screen.getByLabelText("新增頁面目的"), { target: { value: "比較導入前後成效，包含交付時間與失敗率" } });
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ purpose: "比較導入前後成效，包含交付時間與失敗率", afterSlideId: project.slides[0]!.id }),
      }),
    ));
  ...
  it("shows a named style picker instead of asking for an internal style id", async () => {
    ...
    project.slides[0]!.versions = [{
      id: "version-1", imagePath: "assets/generated/slide.png", prompt: "test", providerId: "mock-image",
      model: "mock", parameters: {}, styleVersion: 1, sources: [], createdAt: now,
    }];
    project.slides[0]!.currentVersionId = "version-1";
    const customStyle = { ...createDefaultStyle(now), id: "team-visual", name: "團隊科技風", system: false, referenceImages: [] };
  ...
  it("previews an old image version without creating a new version and switches only on confirmation", async () => {
    ...
    const baseOutline = { purpose: project.slides[0]!.purpose, narrative: "原始敘事", layoutHint: "原始構圖", sourceIds: [] };
    project.slides[0]!.versions = [
      { id: "version-1", imagePath: "assets/generated/version-1.png", prompt: "first", providerId: "mock-image", model: "mock", parameters: {}, styleVersion: 1, sources: [], outlineSnapshot: { ...baseOutline, content: "第一版大綱內容", imagePrompt: "第一版圖片提示" }, createdAt: firstCreatedAt },
      { id: "version-2", imagePath: "assets/generated/version-2.png", prompt: "second", providerId: "mock-image", model: "mock", parameters: {}, styleVersion: 1, sources: [], outlineSnapshot: { ...baseOutline, content: "第二版大綱內容", imagePrompt: "第二版圖片提示" }, createdAt: secondCreatedAt },
    ];
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request) => {
      ...
      if (path.includes("/readiness")) return Response.json({ providerId: "mock-image", status: "ready", blocking: false, requiresAcknowledgement: false, message: "Ready", checkedAt: secondCreatedAt, expiresAt: secondCreatedAt });
      if (path.endsWith("/versions/version-1/activate")) {
        project = structuredClone(project);
        project.slides[0]!.currentVersionId = "version-1";
        Object.assign(project.slides[0]!, project.slides[0]!.versions[0]!.outlineSnapshot, { outlineDirty: false });
        return Response.json(project);
      }
      return Response.json(project);
    });
    ...
    expect(screen.getByRole("img", { name: "Slide 1" }).getAttribute("src")).toContain("version-1.png");
    expect(screen.getByText("正在預覽歷史版本")).toBeTruthy();
    expect(screen.getByDisplayValue("第一版大綱內容")).toHaveProperty("readOnly", true);
    expect(fetchMock.mock.calls.some(([input]) => String(input).endsWith("/versions/version-1/activate"))).toBe(false);
    expect(screen.getAllByRole("button", { name: /^版本 \d/ })).toHaveLength(2);

    fireEvent.click(screen.getByRole("button", { name: "切換至此版本" }));
    await waitFor(() => expect(fetchMock.mock.calls.some(([input]) => String(input).endsWith("/versions/version-1/activate"))).toBe(true));
  ...
  it("does not present the current outline as an old version when the legacy image has no snapshot", async () => {
    ...
    const now = new Date().toISOString();
    project.slides[0]!.versions = [
      { id: "legacy-version", imagePath: "assets/generated/legacy.png", prompt: "legacy", providerId: "mock-image", model: "mock", parameters: {}, styleVersion: 1, sources: [], createdAt: now },
      { id: "current-version", imagePath: "assets/generated/current.png", prompt: "current", providerId: "mock-image", model: "mock", parameters: {}, styleVersion: 1, sources: [], outlineSnapshot: { purpose: project.slides[0]!.purpose, content: project.slides[0]!.content, narrative: project.slides[0]!.narrative, layoutHint: project.slides[0]!.layoutHint, imagePrompt: project.slides[0]!.imagePrompt, sourceIds: [] }, createdAt: now },
    ];
  ...
  it("regenerates only the current slide outline from allowed project sources and marks the draft fields", async () => {
    ...
    slide.versions = [{
      id: "current-version", imagePath: "assets/generated/current.png", prompt: slide.imagePrompt, providerId: "mock-image", model: "mock", parameters: {}, styleVersion: 1, sources: [], createdAt: now,
      outlineSnapshot: { purpose: slide.purpose, content: slide.content, narrative: slide.narrative, layoutHint: slide.layoutHint, imagePrompt: slide.imagePrompt, sourceIds: [] },
    }];
    slide.currentVersionId = "current-version";
    project.sources = [{
      id: "allowed-source", name: "允許來源.md", mediaType: "text/markdown", usage: "content", allowModelAccess: true, status: "indexed",
      assetPath: "assets/sources/allowed.md", sizeBytes: 100, extractedText: "完整證據", chunks: [{ id: "allowed-chunk", text: "完整證據" }], metadata: {}, createdAt: now,
    }, {
      id: "blocked-source", name: "禁止來源.md", mediaType: "text/markdown", usage: "content", allowModelAccess: false, status: "indexed",
      assetPath: "assets/sources/blocked.md", sizeBytes: 100, extractedText: "不可使用", chunks: [{ id: "blocked-chunk", text: "不可使用" }], metadata: {}, createdAt: now,
    }];
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (path.endsWith(`/slides/${slide.id}/outline`)) {
        project = structuredClone(project);
        Object.assign(project.slides[0]!, { content: "加入來源證據後的高密度內容", narrative: "新版敘事", layoutHint: "新版構圖", imagePrompt: "新版圖片提示", sourceIds: ["allowed-source"], outlineDirty: true });
        return Response.json(project);
      }
      if (path.endsWith(`/slides/${slide.id}`) && init?.method === "PATCH") return Response.json(project);
      return Response.json(project);
    });
    vi.stubGlobal("fetch", fetchMock);
    ...
    expect(fetchMock).toHaveBeenCalledWith(expect.stringMatching(/\/outline$/), expect.objectContaining({ method: "POST" }));
  });
  ...
  it("previews text and image sources with model access before the title", async () => {
    ...
    const now = new Date().toISOString();
    project.sources = [{
      id: "text-source", name: "研究摘要.md", mediaType: "text/markdown", usage: "content", allowModelAccess: true,
      status: "indexed", assetPath: "assets/sources/text-source/研究摘要.md", sizeBytes: 2048,
      extractedText: `Grok Build 能縮短從需求到可執行原型的迭代時間。${"完整簡介內容。".repeat(40)}`, chunks: [{ id: "chunk-1", text: "Grok Build 能縮短從需求到可執行原型的迭代時間。" }],
      metadata: {}, createdAt: now, updatedAt: now,
    }, {
      id: "image-source", name: "流程圖.png", mediaType: "image/png", usage: "visual-reference", allowModelAccess: false,
      status: "indexed", assetPath: "assets/sources/image-source/流程圖.png", sizeBytes: 8192,
      extractedText: "", chunks: [], metadata: {}, createdAt: now, updatedAt: now,
    }];
    vi.stubGlobal("fetch", vi.fn(async (input: string | URL | Request) => {
      const path = typeof input === "string" ? input : input instanceof URL ? input.pathname : new URL(input.url).pathname;
      if (path === "/api/projects") return Response.json([project]);
    ...
    expect(title.closest("header")?.firstElementChild?.querySelector("input[type=checkbox]")).toBeTruthy();
    expect(screen.getByText(/Grok Build 能縮短/)).toBeTruthy();
    expect(screen.getByRole("button", { name: "預覽 流程圖.png" }).querySelector("img")).toBeTruthy();
    ...
    expect(dialog.querySelector(".source-preview-intro p")?.textContent?.endsWith("完整簡介內容。")).toBe(true);
  ...
  it("searches web sources and saves only confirmed results back to the project", async () => {
    ...
    const now = new Date().toISOString();
    const results = [
      { url: "https://openai.com/agents", title: "OpenAI Agents Guide", summary: "Agent building guidance." },
      { url: "https://example.com/secondary", title: "Secondary result", summary: "A second result." },
    ];
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      const path = typeof input === "string" ? input : input instanceof URL ? input.pathname : new URL(input.url).pathname;
      if (path === "/api/projects") return Response.json([project]);
      ...
      if (path.endsWith("/web-sources")) {
        ...
        project = structuredClone(project);
        project.sources = [{
          id: "web-source-1", name: "OpenAI Agents Guide.md", mediaType: "text/markdown", usage: "content", allowModelAccess: true,
          status: "indexed", assetPath: "assets/sources/web-source-1/OpenAI Agents Guide.md", sizeBytes: 500,
          extractedText: "# OpenAI Agents Guide\n\n## 全文\n\nComplete captured article.", chunks: [{ id: "chunk-1", text: "Complete captured article." }],
          metadata: { url: results[0]!.url, contentStatus: "full" }, createdAt: now, updatedAt: now,
        }];
        return Response.json(project, { status: 201 });
      }
      return Response.json(project);
    ...
    fireEvent.change(screen.getByLabelText("搜尋關鍵字"), { target: { value: "building AI agents" } });
    fireEvent.click(screen.getByRole("button", { name: "搜尋" }));

    expect(await screen.findByText("OpenAI Agents Guide")).toBeTruthy();
    expect(fetchMock.mock.calls.some(([input]) => String(input).endsWith("/web-sources"))).toBe(false);
    fireEvent.click(screen.getAllByRole("checkbox")[1]!);
    fireEvent.click(screen.getByRole("button", { name: "加入所選來源（1）" }));

    await waitFor(() => expect(fetchMock.mock.calls.some(([input]) => String(input).endsWith("/web-sources"))).toBe(true));
  ...
  it("uploads multiple selected files and creates an indexed source from pasted text", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url.pathname === "/api/providers") return Response.json([{ id: "mock-image", name: "Mock", availability: { status: "available" }, capabilities: { fullSlideGeneration: true } }]);
      if (url.pathname === "/api/styles") return Response.json([createDefaultStyle(now)]);
      if (url.pathname.includes("/readiness")) return Response.json({ providerId: "mock-image", status: "ready", blocking: false, requiresAcknowledgement: false, message: "Ready", checkedAt: now, expiresAt: now });
      if (url.pathname.endsWith("/sources") && init?.method === "POST") {
        const name = url.searchParams.get("name")!;
        uploadedNames.push(name);
        project = structuredClone(project);
        project.sources.push({
          id: `source-${uploadedNames.length}`, name, mediaType: url.searchParams.get("mediaType") ?? "text/markdown", usage: "content", allowModelAccess: true,
          status: "indexed", assetPath: `assets/sources/${uploadedNames.length}/${name}`, sizeBytes: 10, extractedText: name,
          chunks: [{ id: `chunk-${uploadedNames.length}`, text: name }], metadata: {}, createdAt: now, updatedAt: now,
        });
        return Response.json(project, { status: 201 });
      }
      return Response.json(project);
    ...
    fireEvent.change(input, { target: { files: [new File(["one"], "one.md", { type: "text/markdown" }), new File(["two"], "two.txt", { type: "text/plain" })] } });
    ...
    fireEvent.change(screen.getByLabelText("文字來源內容"), { target: { value: "這是直接貼上的完整研究內容。" } });
  ...
  it("changes editing pages with arrow keys and opens keyboard-driven presentation mode", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: string | URL | Request) => {
      ...
      if (path.includes("/readiness")) return Response.json({ providerId: "mock-image", status: "ready", blocking: false, requiresAcknowledgement: false, message: "Ready", checkedAt: new Date().toISOString(), expiresAt: new Date().toISOString() });
      return Response.json(project);
    }));
  ...
  it("opens current-image editing and submits an instruction without replacing the base version", async () => {
    ...
    project.workflowStage = "editing";
    const now = new Date().toISOString();
    project.slides[0]!.versions = [{ id: "base-version", imagePath: "assets/generated/base.png", prompt: "base", providerId: "codex-image-spike", model: "codex-imagegen", parameters: {}, styleVersion: 1, sources: [], createdAt: now }];
    project.slides[0]!.currentVersionId = "base-version";
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      const path = typeof input === "string" ? input : input instanceof URL ? input.pathname : new URL(input.url).pathname;
      if (path === "/api/projects") return Response.json([project]);
      if (path === "/api/providers") return Response.json([{ id: "mock-image", name: "Mock", availability: { status: "available" }, capabilities: { fullSlideGeneration: true, imageEditing: true, maskedEditing: true } }]);
      if (path === "/api/styles") return Response.json([createDefaultStyle(now)]);
      if (path.includes("/readiness")) return Response.json({ providerId: "mock-image", status: "ready", blocking: false, requiresAcknowledgement: false, message: "Ready", checkedAt: now, expiresAt: now });
      if (path.endsWith("/edit-image")) return Response.json({ id: "edit-job", projectId: project.id, slideId: project.slides[0]!.id, providerId: "mock-image", status: "queued", operation: "edit", attempt: 0, createdAt: now, updatedAt: now }, { status: 202 });
      return Response.json(project);
    });
    vi.stubGlobal("fetch", fetchMock);
    ...
    fireEvent.change(screen.getByLabelText("圖片修改說明"), { target: { value: "只調整右上角圖示" } });
    fireEvent.click(screen.getByText("套用修改 →"));
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith(
      expect.stringMatching(/\/edit-image$/),
      expect.objectContaining({ method: "POST", body: expect.stringContaining("只調整右上角圖示") }),
    ));
  });

  it("opens a project and exposes slide, source, project and export workflows", async () => {
    const project = createProject({ topic: "UI 測試專案", brief: { desiredSlideCount: 2, audience: "工程團隊" } });
    project.workflowStage = "editing";
    vi.stubGlobal("fetch", vi.fn(async (input: string | URL | Request) => {
      const path = typeof input === "string" ? input : input instanceof URL ? input.pathname : new URL(input.url).pathname;
      if (path === "/api/projects") return Response.json([project]);
  ...
  it("uses the confirmed outline length and starts full-deck generation immediately after step two", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (path.endsWith("/outline") && method === "POST") {
        project = { ...project, workflowStage: "settings", slides: createSlidesFromBrief(project.brief) };
        return Response.json(project);
      }
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ method: "POST", body: JSON.stringify({ providerId: "mock-image", acceptUnknownReadiness: false }) }),
    ));
    expect(await screen.findByText("SLIDE SPEC")).toBeTruthy();
  });
});
