...
if (mode === "text") {
  ...
  const result = await spawnWithArgv("codex", [
  ...
  if (!result.stdout.includes("SLIDE_MAKER_CODEX_OK")) throw new Error("TEXT_SMOKE_UNEXPECTED_RESPONSE");
  console.log(JSON.stringify({ smoke: "text", status: "passed", eventCount }));
} else if (mode === "image") {
  if (process.env.SLIDE_MAKER_ENABLE_CODEX_SOFT_SANDBOX !== "1") throw new Error("IMAGE_SMOKE_OPT_IN_REQUIRED");
  ...
  const project = createProject({ topic: "A minimal editorial slide with the title: Soft Sandbox Smoke Test" });
  ...
  project.slides[0]!.imagePrompt = "Minimal dark navy 16:9 presentation slide, one warm orange circle, large white title Soft Sandbox Smoke Test, no other text.";
  ...
  if (finalJob?.status !== "completed" || !version) {
    throw new Error(`IMAGE_SMOKE_${finalJob?.status?.toUpperCase() ?? "TIMEOUT"}:${finalJob?.error ?? "NO_RESULT"}`);
  }
  const relativeAssetPath = version.imagePath.startsWith("assets/") ? version.imagePath.slice("assets/".length) : version.imagePath;
