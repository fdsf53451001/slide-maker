...
const refine = (boxes: EditableTextBox[]) =>
  refineOcrBoxes(boxes, { sourceTexts: [SLIDE_CONTENT, LAYOUT_HINT] }).boxes;

/** 建立白底合成畫布，並以純色矩形模擬字墨帶。 */
function raster(
  width: number,
  height: number,
...
describe("source-anchored OCR text correction", () => {
  it("fixes simplified-character misreads and restores designed spacing from the outline", () => {
    const [corrected] = refine([box({ text: "将代理部署至AWS，" })]);
    expect(corrected?.text).toBe("將代理部署至 AWS，");
  });

  it("fixes em-dashes misread as 一", () => {
    const [corrected] = refine([box({ text: "把建置一競賽一診斷一" })]);
    expect(corrected?.text).toBe("把建置—競賽—診斷—");
  });

  it("matches arrow chains from the layout hint", () => {
    const [corrected] = refine([box({ text: "建置→部署 →協作 →診斷 →理解→移轉" })]);
    expect(corrected?.text).toBe("建置→部署→協作→診斷→理解→移轉");
  });

  it("keeps text that has no close match in the sources", () => {
    const [kept] = refine([box({ text: "全然無關的浮水印" })]);
    expect(kept?.text).toBe("全然無關的浮水印");
  });

  it("keeps single-character boxes untouched", () => {
    const [kept] = refine([box({ text: "5" })]);
    expect(kept?.text).toBe("5");
  });
});

describe("merged heading/body box splitting", () => {
  it("splits a heading merged with body copy and re-estimates the body font size", () => {
...
describe("number badge glued to body text by OCR", () => {
  it("splits before correcting so the badge digits are not rewritten into a source neighbour", () => {
    ...
    const { boxes } = refineOcrBoxes(boxesFromOcr(raw, { width: 1920, height: 1080 }, 0.75), {
      sourceTexts: [source],
    });
...
describe("trailing punctuation restore", () => {
  it("appends the source's trailing full-width punctuation and widens the box", () => {
    const footer = box({
      text: "重點：目標不是只贏一場，而是帶走一套能建置、驗證與改進自主系統的方法",
      width: 1562,
      fontSize: 48.4,
    });
    const [corrected] = refine([footer]);
...
describe("ink-based geometry snapping", () => {
  // 實測 renderer metrics：CJK 字墨高 0.94em、字墨頂距框頂 0.12em、左 bearing 0.05em。
  it("derives font size and position from the measured ink instead of the padded detection box", () => {
    // 10 個 CJK 字、字墨 30px 高（≈ 31.9px 字級）；偵測框帶 unclip 外擴（44px 高）。
    ...
    const { boxes } = refineOcrBoxes([detection], { sourceTexts: [], image });
    const snapped = boxes[0]!;
    // 行寬 296px ÷ (10 × 0.99em) ≈ 29.9px，遠小於偵測框換算的 34.3px。
    expect(snapped.fontSize).toBeGreaterThan(28);
    expect(snapped.fontSize).toBeLessThan(32);
    // x/y 反推自字墨位置：x ≈ 50 − 0.05em、y ≈ 40 − 0.12em。
    expect(snapped.x).toBeCloseTo(50 - 0.05 * snapped.fontSize, 0);
    expect(snapped.y).toBeCloseTo(40 - 0.12 * snapped.fontSize, 0);
  });

  it("ignores a neighbouring text line bleeding into the search region", () => {
  ...
  it("drops a thin separator bar at the edge from the width estimate", () => {
  ...
  it("spans the detection box vertically but hugs the ink horizontally (preserving dividers)", () => {
    const image = raster(400, 120, [{ x: 50, y: 40, w: 296, h: 30 }]);
    const detection = box({
      id: "det",
      text: "接入即時比賽環境規劃",
    ...
    const { maskRects } = refineOcrBoxes([detection], { sourceTexts: [], image });
  ...
  it("uses CJK ink metrics for a Latin-majority line that still contains CJK", () => {
    // 「Kimi K3 模型研究」拉丁字元多於 CJK，但字墨高由撐滿 em box 的 CJK 決定。
    // 舊的多數決會判成拉丁、以 0.74 除數把字級高估近三成（實測封面主標 109→138px）。
    ...
    const { boxes } = refineOcrBoxes([detection], { sourceTexts: [], image });
    // 30px 字墨 ÷ 0.94 ≈ 31.9px；若誤用拉丁 0.74 除數會得 ≈ 40.5px。
    expect(boxes[0]!.fontSize).toBeGreaterThan(29);
    expect(boxes[0]!.fontSize).toBeLessThan(34);
  });

  it("falls back to detection geometry when the region has no ink", () => {
...
describe("end-to-end refinement of the slide-8 regression", () => {
  it("corrects, splits, and unifies the real page-8 boxes", () => {
    const boxes = [
      box({
        id: "title",
