...
async function waitForJobs(repository: FileProjectRepository, projectId: string): Promise<void> {
  for (let attempt = 0; attempt < 100; attempt += 1) {
    ...
    if (project?.jobs.every((job) => ["completed", "failed", "cancelled"].includes(job.status))) return;
...
describe("JobRunner provider boundaries", () => {
  it("honors the provider concurrency limit", async () => {
    ...
    const provider: ImageProvider = {
      ...
      async generate(_request: ImageGenerationRequest): Promise<GeneratedImage> {
        ...
        active -= 1;
        return {
          bytes: new TextEncoder().encode('<svg xmlns="http://www.w3.org/2000/svg" width="1920" height="1080"><rect width="1920" height="1080"/></svg>'),
    ...
    const repository = new FileProjectRepository(await mkdtemp(join(tmpdir(), "slide-maker-concurrency-")));
    ...
    expect((await repository.loadProject(project.id))?.jobs.every((job) => job.status === "completed")).toBe(true);
  });
  ...
  it("rejects mismatched provider output before it reaches assets", async () => {
    ...
    const repository = new FileProjectRepository(await mkdtemp(join(tmpdir(), "slide-maker-bad-output-")));
    ...
    expect(loaded?.jobs[0]).toMatchObject({ errorCode: "OUTPUT_VALIDATION_FAILED", error: "生成圖片未通過安全或格式驗證。" });
  });
  ...
  it("rejects invalid provider concurrency before persisting a job", async () => {
    ...
    const repository = new FileProjectRepository(await mkdtemp(join(tmpdir(), "slide-maker-invalid-concurrency-")));
    const project = createProject({ topic: "Invalid concurrency" });
    await repository.saveProject(project);
    const runner = new JobRunner(repository, new ProviderRegistry<ImageProvider>().register(provider));
    await expect(runner.enqueue(project.id, project.slides[0]!.id, provider.id)).rejects.toThrow(/between 1 and 32/);
  ...
  it("cannot cancel another project's job by guessing its id", async () => {
    const provider: ImageProvider = {
      id: "mock-image", name: "Ownership test", availability: { status: "available" }, maxConcurrency: 1, capabilities,
      async generate() {
        await new Promise((resolve) => setTimeout(resolve, 40));
        return {
          bytes: new TextEncoder().encode('<svg xmlns="http://www.w3.org/2000/svg" width="1920" height="1080"><rect width="1920" height="1080"/></svg>'),
          mediaType: "image/svg+xml", extension: "svg", model: "test", parameters: {},
        };
      },
    };
    const repository = new FileProjectRepository(await mkdtemp(join(tmpdir(), "slide-maker-cancel-owner-")));
  ...
  it("does not trust provider-authored safe error text or codes", async () => {
    const provider: ImageProvider = {
      id: "hostile-error", name: "Hostile", availability: { status: "available" }, capabilities,
      async generate() { throw new SafeProviderError("ATTACKER_CODE", "Bearer forged-secret\nforged log"); },
    };
    const repository = new FileProjectRepository(await mkdtemp(join(tmpdir(), "slide-maker-hostile-error-")));
    ...
    expect(job).toMatchObject({ errorCode: "PROVIDER_FAILED", error: "圖片生成失敗，請檢查 provider 狀態後重試。" });
