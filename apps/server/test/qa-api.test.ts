...
describe("QA local API vertical flow", () => {
  ...
  it("creates, edits, generates, serves, and immutably restores one slide", async (context) => {
    ...
    const draftChanged = await json<PresentationProject>(
      `/api/projects/${projectId}/slides/${slideId}`,
      {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ content: "Unsaved next-image draft" }),
      },
    );
  ...
  it("keeps search results temporary until confirmed, then stores captured full text", async (context) => {
    ...
    const created = await json<PresentationProject>("/api/projects", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ topic: "Web source flow" }),
    });
    const projectId = created.body.id;
    const searched = await json<Array<{ url: string; title: string; summary: string }>>(
      `/api/projects/${projectId}/web-search`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ query: "Agent building", limit: 8 }),
      },
    );
    expect(searched.response.status).toBe(200);
    expect(searched.body).toHaveLength(1);
    expect(
      (await json<PresentationProject>(`/api/projects/${projectId}`)).body.sources,
    ).toHaveLength(0);

    const saved = await json<PresentationProject>(`/api/projects/${projectId}/web-sources`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ sources: searched.body }),
    });
