...
function provider(input: {
  ...
  return {
    ...
    async generate(): Promise<GeneratedImage> { throw new Error("preflight must never generate"); },
  };
}

function service(imageProvider: ImageProvider) {
  return new ProviderReadinessService(new ProviderRegistry<ImageProvider>().register(imageProvider));
}
...
describe("QA provider readiness service", () => {
  ...
  it.each([
    ...
    ["unknown", false, true],
  ] as const)("returns a safe allowlisted %s contract", async (status, blocking, requiresAcknowledgement) => {
    const result = await service(provider({ status })).check("qa-readiness");
    expect(result).toMatchObject({ providerId: "qa-readiness", status, blocking, requiresAcknowledgement });
    expect(Object.keys(result).sort()).toEqual([
      "blocking", "checkedAt", "expiresAt", "message", "providerId", "requiresAcknowledgement", "status",
    ]);
    expect(result.message.length).toBeGreaterThan(5);
    expect(JSON.stringify(result)).not.toMatch(/pid|path|prompt|stderr|Bearer|TOKEN-CANARY|base64|revised_prompt/i);
  });

  it("fail-closes a ready CLI when its image artifact contract is unsupported", async () => {
    const result = await service(provider({ status: "ready", artifactContract: "unsupported" })).check("qa-readiness");
    expect(result).toMatchObject({ status: "artifact_unsupported", blocking: true, requiresAcknowledgement: false });
  });
  ...
  it("only allows the explicit unknown-risk acknowledgement and never bypasses blocking states", async () => {
    ...
    await expect(unknownService.assertCanGenerate("qa-readiness", false)).rejects.toBeInstanceOf(ProviderReadinessGateError);
    await expect(unknownService.assertCanGenerate("qa-readiness", true)).resolves.toMatchObject({ status: "unknown" });
